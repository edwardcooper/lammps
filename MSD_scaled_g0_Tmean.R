...
MSD_scaled_g0_one_temp_Tmean=function(path="~/Dropbox/lammps/PS_20/atom300",filename="atom.300_1",core_num=10){
  ...
  timeRecordB()
  NGP.600%>%write.table(file="NGP.g0.Tmean.1.txt", sep=",")
...
# example use 
# MSD_scaled_g0_one_temp_Tmean(path="~/Dropbox/lammps/PS_20/atom300",filename="atom.300_1",core_num = 10)
...
MSD_scaled_g0_Tmean=function(Path="~/Dropbox/lammps/",polymer="PS_20",temperatures=seq(200,520,by=20),core_num=10){
  library(magrittr)
  ...
  for (i in seq_along(temperatures)){
    ...
    # calculation for MSD
    MSD_scaled_g0_one_temp_Tmean(path=path,filename =filename,core_num = core_num )
